...
while action is not None:
    if isinstance(action, CorruptParty):
        ...
        write_instructions, reply = protocol_parties[action.target](action.source, msg)
        ...
        write_instructions, reply = protocol_parties[action.target].write_to_interpreter(action.source, action.msg)
        for port, msg in write_instructions:
            outgoing_buffers[action.target][port].write_message(msg)
        action = adversary.next_action(reply)
    elif isinstance(action, SendOutgoingMessage):
        write_instructions = protocol_parties[action.source].write_to_outgoing_buffer(action.target, action.msg)
        for port, msg in write_instructions:
            outgoing_buffers[action.target][port].write_message(msg)
        action = adversary.next_action(None)
